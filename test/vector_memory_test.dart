...
void main() {
  ...
  test('测试向量存储服务初始化', () async {
    expect(vectorMemoryService != null, true);
  });
  ...
  test('测试添加和检索记忆条目', () async {
    ...
    expect(results[0].content, contains('调查'));
  });
  ...
  test('测试语义相似度计算', () async {
    ...
    expect(results[0].content, contains('电力'));
    expect(results[1].content, contains('停电'));
    expect(results[2].content, contains('天气'));
  });
}
