  ...
  ContinuityState apply(
    ...
    for (final change in transition['changes'] as List) {
      if (change is! Map ||
          ...
          change['from'] != next[change['key']] ||
          change['to'] is! String) {
        throw const FormatException('状态更新必须属于已定义项，并提供匹配的旧值和文本新值');
      }
      // 空字符串是明确清空；遗漏字段才表示不变。
      changed = changed || next[change['key']] != change['to'];
      next[change['key'] as String] = change['to'] as String;
    }
