...
''';
  ...
  static void _writeEventMemorySections(StringBuffer buffer, Contact contact) {
    final short = contact.eventGraph.shortTermQueue.take(10).toList();
    final long = contact.eventGraph.longTermQueue.take(5).toList();
    final history = contact.eventGraph.ultraLongTermQueue.take(2).toList();
  ...
  static void _writeEventNodes(StringBuffer buffer, List<EventNode> nodes) {
    if (nodes.isEmpty) {
      buffer.writeln(' - （无）');
      return;
    }
  ...
  static void _writeEventMemories(
      StringBuffer buffer, List<EventMemory> events) {
    if (events.isEmpty) {
      buffer.writeln(' - （无）');
      return;
    }
  ...
  static void _writeEdgeSections(StringBuffer buffer, Contact contact) {
  ...
  static String composeStructuredOutputPrompt({
  ...
  static String composeSystemPromptWithContactObject({
