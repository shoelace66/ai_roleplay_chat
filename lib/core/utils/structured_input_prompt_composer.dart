...
- 状态定义中 id 是稳定标识，label 是显示名称，type 为 string；defaultValue 仅在没有当前值时使用。有 enum 时，非空新值必须精确取自该选项数组，不得改写选项或输出数组外的值。
...
- stateTransition.baseRevision 必须等于快照 revision。changes 每项包含 key/from/to；key 使用用户定义项的稳定 ID，from 精确复制当前值，to 为文本。无变化省略该项，明确清空使用空字符串，二者不能混淆。
