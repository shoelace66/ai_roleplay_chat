...
为提升语义理解能力，系统还实现了轻量级向量存储服务：

**核心功能**：
- **本地向量存储**：使用 Hive 数据库，无需网络连接
- **向量生成**：将文本转换为高维向量表示
- **相似度计算**：使用余弦相似度计算向量间相似度
- **语义检索**：基于向量相似度返回相关记忆
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| 向量相似度权重 | 80 | 向量数据库相似度匹配的权重 |
| 关键词匹配权重 | 100 | 关键词精确匹配的权重 |
...
- **综合评分**：结合关键词匹配（60%）和语义相似度（40%）计算最终分数

**多级检索策略**：
1. **直接事件匹配**：遍历所有事件，按综合分数排序
2. **关联事件扩展**：基于事件图谱的边关系，查找邻居事件
3. **物品关联检索**：搜索相关物品及其关联事件（LRU排序后）
4. **设定关联检索**：搜索相关设定及其关联事件（LRU排序后）
